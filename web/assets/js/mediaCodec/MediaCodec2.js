...
function song(config, id, title, pSongKey) {
  ...
  var retData = "";
      switch (pObjectType) {
        ...
        retData = {partNames: partNames, partText: partText};
      break;
          case OBJECTTYPE.songPart:
        var partLineArr = [];
        jQuery.each(pJsonData.partData, function () {
      ...
      switch (pObjectType) {
          case OBJECTTYPE.song:
              ...
              xmlDoc.find(XMLTAGS.part).each(function () {
                  retData.parts.push(decodeXML({ partName: $(this).attr("Name"), partEncodedData: $(this) }, OBJECTTYPE.songPart, root));
              });

              break;
          case OBJECTTYPE.songPart:
              retData = { partName: pData.partName, partData: [] };
      ...
      switch (pObjectType) {
          case OBJECTTYPE.song:
              ...
              for (var i = 0; i < partNamesList.length; i++) {
                  ...
                  retData.parts.push(decodeLegacy({ partName: temp, partEncodedData: pData.substring(start, endTagPos) }, OBJECTTYPE.songPart));
              };
              break;
          case OBJECTTYPE.songPart:
              retData = { partName: pData.partName, partData: [] };
  ...
  function decodePlainText(pData, pObjectType,root) {
     var retData = {};
      ...
      switch (pObjectType) {
          ...
          retData.parts.push(decodePlainText({partName: pData.partName[i], partData: pData.partData[i]},OBJECTTYPE.songPart, root));
        };
              break;
          case OBJECTTYPE.songPart:
              retData = { partName: pData.partName, partData: [] };
                    ...
                    if (pData.substring(pos1,1) == ' ') {
                        ...
                        for(pos2=pos1 + 1; pos2 < pData.length; pos2++) {
                            if (pData.substring(pos2,1) !== ' ') {
                                ...
                                retData.note.push({note: temp, position: pos1});
                                pos1 = pos2;
                                temp = '';
                                break;
                            };
...
function transposer(pKey, pTransposeKey) {
  ...
  this.transposeNote = function(pNote) {return getTransposedNote(pNote, this.numTransposeVal, this.noteArray1, this.noteArray2);};
